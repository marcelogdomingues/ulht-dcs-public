...
/**
 * Utility class for validating Sis API responses and handling error codes.
 * 
 * The Sis API returns error codes in the response body even when HTTP status is 200 OK.
 * This validator checks for these error codes and throws appropriate exceptions.
 */
public class SisResponseValidator {
    
    private static final Logger logger = LoggerFactory.getLogger(SisResponseValidator.class);
     ...
     * @throws SisApiException if the error code indicates an error
    ...
    public static void validateResponse(Object response, String errorCode, String operationContext) 
            throws SisApiException, BadRequestException, UnauthorizedException, ForbiddenException, NotFoundException, ValidationException {
        
        // If no error code, assume success
        if (!SisErrorCode.isError(errorCode)) {
            logger.debug("Response validation successful for operation: {}", operationContext);
            return;
        }
        
        SisErrorCode sisError = SisErrorCode.fromCode(errorCode);
        logger.warn("Sis API returned error code {} ({}) for operation: {}", 
                errorCode, sisError.getName(), operationContext);
        
        // Map Sis error codes to our exception hierarchy
        switch (sisError) {
            case OK:
            ...
            case AUTHENTICATION_FAILURE, INVALID_INSTALLATION_KEY:
                throw new UnauthorizedException(String.format("%s: %s (Operation: %s)", 
                        sisError.getName(), sisError.getDescription(), operationContext));
            ...
            case IMEI_ALREADY_REGISTERED, GENERIC_ERROR:
                throw new SisApiException(sisError, operationContext);
