...
The ULHT Lusofona Service (Digital Wallet Service) provides comprehensive integration between ULHT (Universidade Lusófona de Humanidades e Tecnologias) and WaltID digital wallet platform. It enables secure management of student academic data, digital credentials, and seamless integration with the university's information systems (SIGES).
...
2. **Run the application:**
   ```bash
   java -jar target/lusofona-service-0.0.1-SNAPSHOT.jar
   ```
...
1. **Build the Docker image:**
   ```bash
   docker build -t lusofona-service:latest .
   ```

2. **Run the container:**
   ```bash
   docker run -p 8085:8085 lusofona-service:latest
   ```
...
```java
@FeignClient(name = "lusofonaClient", configuration = ClientConfiguration.class)
...
- `LUSOFONA_SERVICE_URL`: SIGES service URL
...
- Documentation: https://docs.ulusofona.pt/digital-wallet
