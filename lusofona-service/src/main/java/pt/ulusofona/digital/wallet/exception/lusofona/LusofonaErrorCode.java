 ...
 * 
 * This enum maps the error codes returned by the Lusofona API to meaningful names and descriptions.
 * The Lusofona API returns these error codes in the response body's errorCode field.
 * 
 * Error Codes:
 * 0 - OK (No error)
 * 1 - Generic error
 ...
 */
public enum LusofonaErrorCode {
    
    OK("0", "Success", "Operation completed successfully"),
    GENERIC_ERROR("1", "Generic Error", "A generic error occurred in the Lusofona system"),
    ...
    DATA_NOT_FOUND("7", "Data Not Found", "The requested data was not found in the Lusofona system"),
    ...
    LusofonaErrorCode(String code, String name, String description) {
        this.code = code;
        this.name = name;
    ...
    /**
     * Get LusofonaErrorCode from string code
     * 
     * @param code the error code string (e.g., "0", "1", "2")
     * @return the LusofonaErrorCode enum value
     * @throws IllegalArgumentException if the code is not recognized
     */
    public static LusofonaErrorCode fromCode(String code) {
        if (code == null || code.trim().isEmpty()) {
            return OK; // Treat null/empty as success
        }
        
        for (LusofonaErrorCode errorCode : values()) {
