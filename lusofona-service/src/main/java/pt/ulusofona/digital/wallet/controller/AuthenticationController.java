...
import pt.ulusofona.digital.wallet.generated.api.AuthenticationApi;
import pt.ulusofona.digital.wallet.generated.model.*;
import pt.ulusofona.digital.wallet.service.LusofonaService;
import pt.ulusofona.digital.wallet.kafka.CredentialWorkflowProducer;
import pt.ulusofona.digital.wallet.exception.http.ExternalServiceException;
...
public class AuthenticationController implements AuthenticationApi {
    ...
    public AuthenticationController(LusofonaService lusofonaService, 
    ...
    public ResponseEntity<LoginResponse> studentLogin(@Valid StudentServiceRequest studentServiceRequest) {
        ...
        try {
            ...
            // 2. Get student data from external ULHT services (NO MOCK DATA FALLBACK!)
            log.info("Calling ULHT API for student data: {}", studentServiceRequest.getUserName());
            Object studentData = lusofonaService.login(requestMap);
            log.info("✅ Got real student data from ULHT API");
            ...
            // CRITICAL: ULHT API failed - DO NOT issue credentials with fake data!
            log.error("❌ ULHT API failed for user {}: {} - Request will FAIL", 
                     studentServiceRequest.getUserName(), e.getMessage(), e);
            
            // Throw specific exception (handled by GlobalExceptionHandler)
            throw new ExternalServiceException(
                "Failed to retrieve student data from ULHT API: " + e.getMessage(), e);
        }
    }
    ...
    public ResponseEntity<RegistrationResponse> studentRegistration(@Valid StudentRegistrationRequest studentRegistrationRequest) {
        ...
        try {
            ...
            lusofonaService.registration(requestMap);
