 ...
 * 
 * This enum provides a centralized way to manage error codes across the ULHT system.
 * Each error code has:
...
public enum ErrorCodes {
    ...
    // Lusofona API Error Codes (mapped from external API)
    LUSOFONA_OK("LUSOFONA-000", "All Good", "Operation completed successfully in Lusofona system"),
    LUSOFONA_GENERIC_ERROR("LUSOFONA-001", "Mystery Box", "A generic error occurred in the Lusofona system"),
    LUSOFONA_AUTH_FAILURE("LUSOFONA-002", "Wrong Password", "Authentication failed - invalid credentials or session expired in Lusofona system"),
    LUSOFONA_IMEI_REGISTERED("LUSOFONA-003", "Déjà Vu Device", "Device IMEI already registered (legacy error code for backward compatibility)"),
    LUSOFONA_APP_NOT_REGISTERED("LUSOFONA-004", "Missing Membership", "Application is not registered for this user in Lusofona system"),
    LUSOFONA_OPERATION_UNAVAILABLE("LUSOFONA-005", "Access Denied", "This operation is not available for the user's profile or role in Lusofona system"),
    LUSOFONA_INCORRECT_FORMAT("LUSOFONA-006", "Garbled Message", "Request attribute has incorrect format or invalid data type for Lusofona API"),
    LUSOFONA_DATA_NOT_FOUND("LUSOFONA-007", "Empty Locker", "The requested data was not found in the Lusofona system"),
    LUSOFONA_INVALID_KEY("LUSOFONA-008", "Expired Ticket", "The installation key provided is invalid or expired for Lusofona system"),
    LUSOFONA_INVALID_REQUEST("LUSOFONA-009", "Broken Package", "The request is invalid or malformed for Lusofona API"),
    LUSOFONA_BUSINESS_RULE("LUSOFONA-010", "Rule Breaker", "Business rule validation failed in Lusofona system");
