...
import com.example.dcs.sis.domain.student.StudentCourseCredits;
import com.example.dcs.sis.domain.student.StudentEnrolments;
import com.example.dcs.sis.domain.student.StudentEvals;
import com.example.dcs.sis.domain.student.StudentGrades;
...
/**
 * DEMO profile configuration for the sis-service.
 *
 * <p>When the {@code demo} Spring profile is active, this configuration provides a
 * {@link Primary @Primary} in-memory implementation of {@link SisClient} that returns
 ...
 *
 * <p>The real Feign-generated {@code SisClient} bean is left untouched; because this bean
...
public class DemoConfiguration {
    ...
    /**
     * A {@link Primary @Primary} mock of {@link SisClient} that returns canned SIS data.
     *
     * <p>The Kafka credential-issuance flow only calls {@link SisClient#login(Object)} and
    ...
    @Bean
    @Primary
    public SisClient demoSisClient() {
        log.warn("⚠️  DEMO profile active - using in-memory mock SisClient (no real SIS calls). "
                + "Student data is illustrative and obviously fake.");
        return new DemoSisClient();
    }

    /**
     * In-memory implementation of {@link SisClient} returning canned demo data.
     */
    static final class DemoSisClient implements SisClient {
