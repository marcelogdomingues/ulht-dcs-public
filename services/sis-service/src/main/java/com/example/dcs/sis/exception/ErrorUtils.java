...
public class ErrorUtils {
    ...
    public static String getUserFriendlyMessage(ErrorCodes errorCode) {
        return switch (errorCode) {
            ...
            // Sis API Error Codes
            case SIS_OK -> "Operation completed successfully";
            case SIS_GENERIC_ERROR -> "A generic error occurred in the Sis system";
            case SIS_AUTH_FAILURE -> "Authentication failed - invalid credentials or expired session";
            case SIS_IMEI_REGISTERED -> "Device already registered in the Sis system";
            case SIS_APP_NOT_REGISTERED -> "Application is not registered for this user";
            case SIS_OPERATION_UNAVAILABLE -> "This operation is not available for your user profile";
            case SIS_INCORRECT_FORMAT -> "Request has incorrect format or invalid data";
            case SIS_DATA_NOT_FOUND -> "The requested data was not found in the Sis system";
            case SIS_INVALID_KEY -> "The installation key is invalid or expired";
            case SIS_INVALID_REQUEST -> "The request is invalid or malformed";
            case SIS_BUSINESS_RULE -> "Business rule validation failed";
        };
    }
} 